...
const argv = yargs(hideBin(process.argv))
  ...
  .help()
  .argv as { userCount: number; eventCount: number; drivewayCount: number };

function runScript(script: string, args: string[]): Promise<void> {
  return new Promise((resolve, reject) => {
    const proc = spawn('npx', ['ts-node', script, ...args], { stdio: 'inherit' });
    proc.on('close', code => {
      if (code === 0) resolve();
...
(async () => {
  try {
    ...
    await runScript('scripts/seed-users.ts', ['--count', argv.userCount.toString()]);
    console.log('Seeding events...');
    await runScript('scripts/seed-events.ts', ['--count', argv.eventCount.toString()]);
    console.log('Seeding driveways...');
    await runScript('scripts/seed-driveways.ts', ['--count', argv.drivewayCount.toString()]);
